...
=head2 Code structure: packages

GNU B<parallel> is divided into these packages inside the single file:

=over 2

=item B<main>

Parses options, initializes globals, and drives the top-level execution
loop (B<run_gnu_parallel()>, B<drain_job_queue()>).

=item B<SSHLogin>

Represents one execution target - either a remote host (via SSH) or
the local machine (the special sshlogin B<:>). Stores the SSH command,
credentials, host-group membership, number of parallel slots, and
runtime state (jobs running, load, last-used time). The string format
it accepts is:

  [@grp+grp/][N/][sshcmd ][[user][:pass]@][host[:port]]

=item B<JobQueue>

A queue of B<Job> objects. Wraps B<CommandLineQueue>, converting
B<CommandLine> objects into B<Job> objects on demand. Supports
B<unget()> so that jobs that cannot start yet (e.g. because the load
is too high) can be pushed back without loss.

=item B<Job>

Represents a single job invocation. Manages the full lifecycle:
process spawning (via B<IPC::Open3>), stdin pipe writing (B<--pipe>
mode), stdout/stderr collection and forwarding, file transfer and
cleanup (B<--transfer>/B<--return>/B<--cleanup>), timeout tracking,
retry logic, and exit status recording.

=item B<CommandLine>

Represents one fully-instantiated command line: the command template
with all replacement strings (B<{}>, B<{/}>, B<{#}>, B<{= =}>, etc.)
resolved against a specific set of input arguments. Constructed by
B<CommandLineQueue::get()> and consumed by B<Job::new()>.

=item B<CommandLineQueue>

Generates B<CommandLine> objects from the command template and one or
more input sources (B<:::>, B<::::>, B<:::+>, etc.). Handles
B<--link>, B<-N>, B<-m>, B<-X>, and all the combinatorics of multiple
input sources.

=item B<Limits::Command>

Determines the maximum command-line length for the current host (or a
remote SSH host). Binary-searches B<ARG_MAX>, caches the result on
disk, and adjusts for B<tmux> title limits. Used by B<CommandLineQueue>
when packing multiple arguments with B<-m> or B<-X>.

=item B<RecordQueue>

Reads records from one input source, splitting on the record separator.

=item B<RecordColQueue>

Like B<RecordQueue>, but also parses CSV columns (for B<--csv>).

=item B<SQLRecordQueue>

Reads input records from an SQL table (for B<--sqlworker>).

=item B<MultifileQueue>

Multiplexes several B<RecordQueue>s, one per B<:::> or B<::::>
argument source.

=item B<Arg>

Wraps a single input argument value throughout its lifetime.
Parses any trailing B<@hostgroup> suffix when B<--hostgroups> is
active. Provides quoting helpers used by B<CommandLine> to embed
the value safely in shell commands.

=item B<TimeoutQueue>

Tracks running jobs that have a timeout (B<--timeout>). Implements
the percentile timeout using a remedian (approximate median) data
structure so that all run times need not be kept in memory.

=item B<SQL>

Database backend for B<--sqlandworker> / B<--sqlmaster> /
B<--sqlworker>. Parses a DB URL (B<sqlite:///path>, B<pg://host/db>,
B<csv:///dir>, ...), connects via B<DBI>, and manages the
job-tracking table.

=item B<Semaphore>

Implements the semaphore mode (B<sem> / B<parallel --semaphore>).
Uses a lock file and advisory locking to limit concurrency across
unrelated processes.
...
The decoder (B<base64_zip_eval()>) looks like this:
...
There is also a simpler B<base64_eval()> variant (no bzip2) used when
the line is between the plain-text limit and the bzip2 threshold, and
a B<base64_wrap()> function that chooses the right encoding and builds
the complete shell command with the decoder prepended.
...
B<--fast> is an execution path that avoids spawning one process per
job. Instead jobs are batched into shell script chunks. Each chunk
contains multiple jobs that run sequentially inside one B</bin/sh>
process (override with B<$PARALLEL_SHELL>). Chunks are separated by
the sentinel string C<"\n#\0\n"> and fed to an inner
C<parallel --pipe -N1> that executes B<-j> chunks in parallel.
The inner B<parallel> sees each chunk as a single job, so grouping
and ordering happen at the chunk level, not the individual-job level.

The chunk size starts at 100 jobs and grows 1.5x each round (capped at
250) to keep chunks small enough that all worker slots stay busy. The
inner B<parallel> uses C<--block 512000>.

Shell helper functions injected into every chunk:

=over 4

=item C<_error()>

Accumulates failure count in C<$_exit>. Defined only when B<--halt>
is active; each job is wrapped as C<( cmd ) || _error>.

=item C<_tagger()>

Awk stream that reads marker lines to switch the tag string and
prefixes all other lines. Used with B<--tag>/B<--tagstring>. Two
named fifos (immediately unlinked after creation) route stdout and
stderr through separate awk processes, giving correct per-job
labelling even inside a multi-job chunk.

=back

The fast path (C<$fp_fh> set) reads input lines directly and does
string substitution without creating Job objects, giving ~0.1 ms/job.
When the fast path is not eligible (non-trivial replacement strings,
multiple input sources, etc.) the normal job-queue machinery builds
the command strings (~0.5 ms/job) but the same chunk-based executor
is used.

See B<parallel --fast --help> for the full option support matrix.

=head3 Fast mode buffer parameter tuning

Three parameters govern chunk sizing: C<pipe_block> (the C<--block>
given to inner B<parallel --pipe>), C<buf_growth> (the multiplier
applied after each chunk is sent), and C<buf_cap> (the maximum chunk
size in jobs).

Two benchmarks were run to find the best values:

B<Benchmark 1 — fast jobs (C<echo>, ~0 ms/job):> 25 combinations of
C<pipe_block> (64000–1048576 bytes) × C<buf_growth> (1.2–5.0) across
seven job-count scales (1 000–1 000 000), five runs each.  Best:
C<pipe_block=512000, buf_growth=5.0, buf_cap=100000>.

B<Benchmark 2 — slow jobs (C<sleep 0.001>, ~1 ms/job, 4-core machine):>
25 combinations of C<buf_growth> (1.2–5.0) × C<buf_cap> (100–2500)
for 1 000 and 10 000 jobs, three runs each.  Best: C<buf_growth=1.5,
buf_cap=250>.

The chosen defaults B<buf_growth=1.5, buf_cap=250> optimise for
real-world jobs (≥1 ms each) on 4–20 cores by generating many small,
balanced chunks.  The growth sequence 100 → 150 → 225 → 337 → 337 → …
produces ~32 chunks for 10 000 jobs, keeping all cores busy across
the full run.

The table below shows median elapsed time (seconds) for the slow-job
benchmark.  "Best" is the fastest of all 25 combinations at that job
count; "chosen" is C<buf_growth=1.5, buf_cap=250>.

  Jobs    Best(s)  Chosen(s)  Overhead
  ------  -------  ---------  --------
   1 000    3.824      4.201      +9.9%
  10 000   28.802     28.802      +0.0%  <- primary target
