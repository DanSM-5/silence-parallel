...
B<--fast> / B<-D fast> is an experimental execution path that avoids
spawning one process per job. Instead jobs are batched into shell
script chunks separated by the sentinel string C<"\n#\0\n">, and the
batch is piped to a child C<parallel --pipe> instance for parallel
execution.

The batch size starts at 100 and grows 3x each round (capped at 100 000)
to amortise overhead for large queues. Shell helper functions
(C<error()>, C<_tagger()>, C<_taggerwrap()>) are injected into every
chunk so that tagging and error counting work without extra processes.

Limitations: B<--group>, B<--halt>, B<--tag>, and B<$PARALLEL_JOBSLOT>
are not yet supported in fast mode.
