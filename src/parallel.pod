...
is updated with the latest output from the job.

The screen keeps the oldest running job in view, so younger jobs may
no be visible. They will, however, be shown when the oldest job
finishes.

Running this example makes it easer to understand:
...
=item B<--sql-master> I<DBURL> (beta testing)
...
=item B<--sql-and-worker> I<DBURL> (beta testing)
...
=item B<--sql-worker> I<DBURL> (beta testing)
